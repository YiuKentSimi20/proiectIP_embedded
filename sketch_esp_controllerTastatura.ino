...
const char* ssid = "607C"; // LinksysB019
const char* password = "Bursuceii"; // wm83y3fby4
...
const char* web_url_status = "http://192.168.0.103:3001/update-status";
...
void loop(void){
  ...
  if (!serial_input.equals("")) {
    ...
    if(statusRobot.startsWith("Status:")) {

        HTTPClient http_web;
        WiFiClient client;
      
        http_web.begin(client, web_url_status);
        http_web.addHeader("Content-Type", "application/json");

        String status = statusRobot.substring(statusRobot.indexOf(':') + 2);
